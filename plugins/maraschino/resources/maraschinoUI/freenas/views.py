...
def start(request, plugin_id):
    (maraschino_key,
    maraschino_secret) = utils.get_maraschino_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=maraschino_key,
        secret=maraschino_secret)
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.maraschino_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
