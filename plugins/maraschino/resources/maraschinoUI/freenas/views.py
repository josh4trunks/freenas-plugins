...
from maraschinoUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (maraschino_key, maraschino_secret) = utils.get_maraschino_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=maraschino_key, secret=maraschino_secret)
    ...
    try:
        maraschino = models.Maraschino.objects.order_by('-id')[0]
        maraschino.enable = True
        maraschino.save()
    except IndexError:
        maraschino = models.Maraschino.objects.create(enable=True)

    try:
        form = forms.MaraschinoForm(maraschino.__dict__, instance=maraschino, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Maraschino data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.maraschino_control
...
def stop(request, plugin_id):
    ...
    try:
        maraschino = models.Maraschino.objects.order_by('-id')[0]
        maraschino.enable = False
        maraschino.save()
    except IndexError:
        maraschino = models.Maraschino.objects.create(enable=False)

    try:
        form = forms.MaraschinoForm(maraschino.__dict__,
            instance=maraschino,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    ...
    cmd = "%s onestop" % utils.maraschino_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Maraschino object
    If it does not exist create a new entry
    """
    try:
        maraschino = models.Maraschino.objects.order_by('-id')[0]
    except IndexError:
        maraschino = models.Maraschino.objects.create()
    ...
    if request.method == "GET":
        form = forms.MaraschinoForm(instance=maraschino, jail_path=jail_path)
    ...
    form = forms.MaraschinoForm(request.POST, instance=maraschino, jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.maraschino_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Maraschino settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('maraschino_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'maraschino', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.maraschino_control, "onestatus"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.maraschino_icon, 'rb') as f:
        icon = f.read()
