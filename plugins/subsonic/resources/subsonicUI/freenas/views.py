...
def start(request, plugin_id):
    (subsonic_key,
    subsonic_secret) = utils.get_subsonic_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=subsonic_key,
        secret=subsonic_secret)
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.subsonic_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
