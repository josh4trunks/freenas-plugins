...
from subsonicUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (subsonic_key, subsonic_secret) = utils.get_subsonic_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=subsonic_key, secret=subsonic_secret)
    ...
    try:
        subsonic = models.Subsonic.objects.order_by('-id')[0]
    ...
    try:
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Subsonic data did not validate, configure '
                'it first.'),
...
def stop(request, plugin_id):
    ...
    try:
        ...
        subsonic = models.Subsonic.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.subsonic_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Subsonic object
    If it does not exist create a new entry
    """
    try:
        subsonic = models.Subsonic.objects.order_by('-id')[0]
    except IndexError:
        subsonic = models.Subsonic.objects.create()
	...
	if subsonic.subsonic_ssl:
		subsonic_scheme = "https"
	else:
		subsonic_scheme = "http"
	subsonic_port = subsonic.subsonic_port
	subsonic_webroot = subsonic.subsonic_context_path
        auth = server.plugins.is_authenticated(
            request.COOKIES.get("sessionid", "")
            )
        assert auth
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('subsonic_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'subsonic', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/local/etc/rc.d/subsonic", "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.subsonic_icon, 'rb') as f:
        icon = f.read()
