...
from sickrageUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (sickrage_key,
    sickrage_secret) = utils.get_sickrage_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=sickrage_key,
        secret=sickrage_secret)
    ...
    try:
        sickrage = models.SickRage.objects.order_by('-id')[0]
        sickrage.enable = True
        sickrage.save()
    except IndexError:
        sickrage = models.SickRage.objects.create(enable=True)

    try:
        form = forms.SickRageForm(sickrage.__dict__,
            instance=sickrage,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('SickRage data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.sickrage_control
...
def stop(request, plugin_id):
    ...
    try:
        sickrage = models.SickRage.objects.order_by('-id')[0]
        sickrage.enable = False
        sickrage.save()
    except IndexError:
        sickrage = models.SickRage.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.sickrage_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def edit(request, plugin_id):
    ...
    """
    Get the SickRage object
    If it does not exist create a new entry
    """
    try:
        sickrage = models.SickRage.objects.order_by('-id')[0]
    except IndexError:
        sickrage = models.SickRage.objects.create()
    ...
    if request.method == "GET":
        form = forms.SickRageForm(instance=sickrage,
            jail_path=jail_path)
    ...
    form = forms.SickRageForm(request.POST,
        instance=sickrage,
        jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.sickrage_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
            shell=True, close_fds=True)

        return JsonResponse(request, error=True,
            message="SickRage settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('sickrage_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'sickrage', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.sickrage_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.sickrage_icon, 'rb') as f:
        icon = f.read()
