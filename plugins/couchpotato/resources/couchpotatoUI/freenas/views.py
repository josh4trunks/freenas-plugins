...
def start(request, plugin_id):
    (couchpotato_key,
    couchpotato_secret) = utils.get_couchpotato_oauth_creds()
    ...
    try:
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('CouchPotato data did not validate, configure '
                'it first.'),
...
def edit(request, plugin_id):
    ...
    if request.method == "GET":
        form = forms.CouchPotatoForm(instance=couchpotato, jail_path=jail_path)
    ...
    form = forms.CouchPotatoForm(request.POST, instance=couchpotato, jail_path=jail_path)
    if form.is_valid():
        form.save()
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.couchpotato_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
