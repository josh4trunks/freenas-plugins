...
from nextcloudUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (nextcloud_key,
    nextcloud_secret) = utils.get_nextcloud_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=nextcloud_key,
    ...
    try:
        nextcloud = models.Nextcloud.objects.order_by('-id')[0]
        nextcloud.enable = True
        nextcloud.save()
    except IndexError:
        nextcloud = models.Nextcloud.objects.create(enable=True)

    try:
        form = forms.NextcloudForm(nextcloud.__dict__,
            instance=nextcloud,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Nextcloud data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.nextcloud_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        nextcloud = models.Nextcloud.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.nextcloud_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('nextcloud_open', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'nextcloud', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.nextcloud_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.nextcloud_icon, 'rb') as f:
        icon = f.read()
