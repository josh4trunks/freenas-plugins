...
def start(request, plugin_id):
    ...
    try:
        ...
        headphones = models.Headphones.objects.create(enable=True)

    try:
        form = forms.HeadphonesForm(headphones.__dict__, instance=headphones, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Headphones data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.headphones_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        headphones = models.Headphones.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.headphones_control
...
def edit(request, plugin_id):
    ...
    if request.method == "GET":
        form = forms.HeadphonesForm(instance=headphones, jail_path=jail_path)
    ...
    form = forms.HeadphonesForm(request.POST, instance=headphones, jail_path=jail_path)
    if form.is_valid():
        form.save()
        ...
        return JsonResponse(request, error=True, message="Headphones settings successfully saved.")
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/local/etc/rc.d/headphones", "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.headphones_icon, 'rb') as f:
        icon = f.read()
