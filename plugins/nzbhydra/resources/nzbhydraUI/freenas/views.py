...
from nzbhydraUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (nzbhydra_key,
    nzbhydra_secret) = utils.get_nzbhydra_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=nzbhydra_key,
        secret=nzbhydra_secret)
    ...
    try:
        nzbhydra = models.NZBHydra.objects.order_by('-id')[0]
        nzbhydra.enable = True
        nzbhydra.save()
    except IndexError:
        nzbhydra = models.NZBHydra.objects.create(enable=True)

    try:
        form = forms.NZBHydraForm(nzbhydra.__dict__,
            instance=nzbhydra,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('NZBHydra data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.nzbhydra_control
...
def stop(request, plugin_id):
    ...
    try:
        nzbhydra = models.NZBHydra.objects.order_by('-id')[0]
        nzbhydra.enable = False
        nzbhydra.save()
    except IndexError:
        nzbhydra = models.NZBHydra.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.nzbhydra_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('nzbhydra_open', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'nzbhydra', 'plugin_id': plugin_id },
    }
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.nzbhydra_icon, 'rb') as f:
        icon = f.read()
