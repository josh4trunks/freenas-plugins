...
from sonarrUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (sonarr_key,
    sonarr_secret) = utils.get_sonarr_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=sonarr_key,
        secret=sonarr_secret)
    ...
    try:
        sonarr = models.Sonarr.objects.order_by('-id')[0]
        sonarr.enable = True
        sonarr.save()
    except IndexError:
        sonarr = models.Sonarr.objects.create(enable=True)

    try:
        form = forms.SonarrForm(sonarr.__dict__,
            instance=sonarr,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Sonarr data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.sonarr_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        sonarr = models.Sonarr.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.sonarr_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def edit(request, plugin_id):
    ...
    """
    Get the Sonarr object
    If it does not exist create a new entry
    """
    try:
        sonarr = models.Sonarr.objects.order_by('-id')[0]
    except IndexError:
        sonarr = models.Sonarr.objects.create()
    ...
    if request.method == "GET":
        form = forms.SonarrForm(instance=sonarr,
            jail_path=jail_path)
    ...
    form = forms.SonarrForm(request.POST,
        instance=sonarr,
        jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.sonarr_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
            shell=True, close_fds=True)

        return JsonResponse(request, error=True,
            message="Sonarr settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('sonarr_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'sonarr', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.sonarr_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.sonarr_icon, 'rb') as f:
        icon = f.read()
