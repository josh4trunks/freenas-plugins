...
from pydioUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (pydio_key,
    pydio_secret) = utils.get_pydio_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=pydio_key,
        secret=pydio_secret)
    ...
    try:
        pydio = models.Pydio.objects.order_by('-id')[0]
        pydio.enable = True
        pydio.save()
    except IndexError:
        pydio = models.Pydio.objects.create(enable=True)

    try:
        form = forms.PydioForm(pydio.__dict__,
            instance=pydio,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Pydio data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.pydio_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        pydio = models.Pydio.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.pydio_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def edit(request, plugin_id):
    ...
    """
    Get the Pydio object
    If it does not exist create a new entry
    """
    try:
        pydio = models.Pydio.objects.order_by('-id')[0]
    except IndexError:
        pydio = models.Pydio.objects.create()
    ...
    if request.method == "GET":
        form = forms.PydioForm(instance=pydio,
            jail_path=jail_path)
    ...
    form = forms.PydioForm(request.POST,
        instance=pydio,
        jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.pydio_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
            shell=True, close_fds=True)

        return JsonResponse(request, error=True,
            message="Pydio settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('pydio_open', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'pydio', 'plugin_id': plugin_id },
    }
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.pydio_icon, 'rb') as f:
        icon = f.read()
