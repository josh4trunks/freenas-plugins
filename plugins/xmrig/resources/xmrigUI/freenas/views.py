...
from xmrigUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (xmrig_key,
    xmrig_secret) = utils.get_xmrig_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=xmrig_key,
        secret=xmrig_secret)
    ...
    try:
        xmrig = models.XMRig.objects.order_by('-id')[0]
        xmrig.enable = True
        xmrig.save()
    except IndexError:
        xmrig = models.XMRig.objects.create(enable=True)

    try:
        form = forms.XMRigForm(xmrig.__dict__,
            instance=xmrig,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('XMRig data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.xmrig_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        xmrig = models.XMRig.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.xmrig_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def edit(request, plugin_id):
    ...
    """
    Get the XMRig object
    If it does not exist create a new entry
    """
    try:
        xmrig = models.XMRig.objects.order_by('-id')[0]
    except IndexError:
        xmrig = models.XMRig.objects.create()
    ...
    if request.method == "GET":
        form = forms.XMRigForm(instance=xmrig,
            jail_path=jail_path)
    ...
    form = forms.XMRigForm(request.POST,
        instance=xmrig,
        jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.xmrig_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
            shell=True, close_fds=True)

        return JsonResponse(request, error=True,
            message="XMRig settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('xmrig_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'xmrig', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.xmrig_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.xmrig_icon, 'rb') as f:
        icon = f.read()
