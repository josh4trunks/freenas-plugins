...
from syncthingUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (syncthing_key,
    syncthing_secret) = utils.get_syncthing_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=syncthing_key,
        secret=syncthing_secret)
    ...
    try:
        syncthing = models.Syncthing.objects.order_by('-id')[0]
        syncthing.enable = True
        syncthing.save()
    except IndexError:
        syncthing = models.Syncthing.objects.create(enable=True)

    try:
        form = forms.SyncthingForm(syncthing.__dict__, instance=syncthing, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Syncthing data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.syncthing_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        syncthing = models.Syncthing.objects.create(enable=False)

    try:
        form = forms.SyncthingForm(syncthing.__dict__,
            instance=syncthing,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    ...
    cmd = "%s onestop" % utils.syncthing_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Syncthing object
    If it does not exist create a new entry
    """
    try:
        syncthing = models.Syncthing.objects.order_by('-id')[0]
    except IndexError:
        syncthing = models.Syncthing.objects.create()
    ...
    if request.method == "GET":
        form = forms.SyncthingForm(instance=syncthing, jail_path=jail_path)
    ...
    form = forms.SyncthingForm(request.POST, instance=syncthing, jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.syncthing_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Syncthing settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('syncthing_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'syncthing', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.syncthing_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.syncthing_icon, 'rb') as f:
        icon = f.read()
