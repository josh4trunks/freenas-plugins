...
def start(request, plugin_id):
    ...
    try:
        lazylibrarian = models.LazyLibrarian.objects.order_by('-id')[0]
    ...
    try:
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('LazyLibrarian data did not validate, configure '
                'it first.'),
...
def stop(request, plugin_id):
    ...
    try:
        lazylibrarian = models.LazyLibrarian.objects.order_by('-id')[0]
        lazylibrarian.enable = False
        lazylibrarian.save()
    except IndexError:
        lazylibrarian = models.LazyLibrarian.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.lazylibrarian_control
...
def edit(request, plugin_id):
    ...
    try:
        ...
        lazylibrarian = models.LazyLibrarian.objects.create()
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('lazylibrarian_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'lazylibrarian', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.lazylibrarian_control, "onestatus"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.lazylibrarian_icon, 'rb') as f:
        icon = f.read()
