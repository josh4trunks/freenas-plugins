...
def start(request, plugin_id):
    (htpcmanager_key,
    htpcmanager_secret) = utils.get_htpcmanager_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=htpcmanager_key,
        secret=htpcmanager_secret)
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.htpcmanager_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
