...
from htpcmanagerUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (htpcmanager_key, htpcmanager_secret) = utils.get_htpcmanager_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=htpcmanager_key, secret=htpcmanager_secret)
    ...
    try:
        htpcmanager = models.HTPCManager.objects.order_by('-id')[0]
        htpcmanager.enable = True
        htpcmanager.save()
    except IndexError:
        htpcmanager = models.HTPCManager.objects.create(enable=True)

    try:
        form = forms.HTPCManagerForm(htpcmanager.__dict__, instance=htpcmanager, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('HTPC-Manager data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.htpcmanager_control
...
def stop(request, plugin_id):
    ...
    try:
        htpcmanager = models.HTPCManager.objects.order_by('-id')[0]
        htpcmanager.enable = False
        htpcmanager.save()
    except IndexError:
        htpcmanager = models.HTPCManager.objects.create(enable=False)

    try:
        form = forms.HTPCManagerForm(htpcmanager.__dict__,
            instance=htpcmanager,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
    ...
    cmd = "%s onestop" % utils.htpcmanager_control
...
def edit(request, plugin_id):
    ...
    """
    Get the HTPC-Manager object
    If it does not exist create a new entry
    """
    try:
        htpcmanager = models.HTPCManager.objects.order_by('-id')[0]
    except IndexError:
        htpcmanager = models.HTPCManager.objects.create()
    ...
    if request.method == "GET":
        form = forms.HTPCManagerForm(instance=htpcmanager, jail_path=jail_path)
    ...
    form = forms.HTPCManagerForm(request.POST, instance=htpcmanager, jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.htpcmanager_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="HTPC-Manager settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('htpcmanager_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'htpcmanager', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.htpcmanager_control, "onestatus"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.htpcmanager_icon, 'rb') as f:
        icon = f.read()
