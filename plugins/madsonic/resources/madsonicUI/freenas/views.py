...
def start(request, plugin_id):
    (madsonic_key,
    madsonic_secret) = utils.get_madsonic_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=madsonic_key,
        secret=madsonic_secret)
...
def edit(request, plugin_id):
    ...
    if request.method == "GET":
        form = forms.MadsonicForm(instance=madsonic,
            jail_path=jail_path)
        return render(request, "edit.html", {
            ...
            'webroot': madsonic.madsonic_context_path
        })

    if not request.POST:
        return JsonResponse(request, error=True, message="A problem occurred.")

    form = forms.MadsonicForm(request.POST,
        instance=madsonic,
        jail_path=jail_path)
    if form.is_valid():
        form.save()
        ...
        return JsonResponse(request, error=True,
            message="Madsonic settings successfully saved.")
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.madsonic_control, "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
