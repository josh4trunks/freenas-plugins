...
from owncloudUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    ...
    try:
        ...
        owncloud = models.Owncloud.objects.create(enable=True)

    try:
        form = forms.OwncloudForm(owncloud.__dict__,
            instance=owncloud,
            jail_path=jail_path)
        form.is_valid()
        form.save()
        ...
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Owncloud data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.owncloud_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        owncloud = models.Owncloud.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.owncloud_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.owncloud_icon, 'rb') as f:
        icon = f.read()
