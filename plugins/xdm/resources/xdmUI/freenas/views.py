...
from xdmUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (xdm_key, xdm_secret) = utils.get_xdm_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=xdm_key, secret=xdm_secret)
    ...
    try:
        ...
        xdm = models.XDM.objects.create(enable=True)

    try:
        form = forms.XDMForm(xdm.__dict__, instance=xdm, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('XDM data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.xdm_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        xdm = models.XDM.objects.create(enable=False)

    try:
        form = forms.XDMForm(xdm.__dict__,
            instance=xdm,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    ...
    cmd = "%s onestop" % utils.xdm_control
...
def edit(request, plugin_id):
    ...
    if request.method == "GET":
        form = forms.XDMForm(instance=xdm, jail_path=jail_path)
    ...
    form = forms.XDMForm(request.POST, instance=xdm, jail_path=jail_path)
    if form.is_valid():
        form.save()
        ...
        return JsonResponse(request, error=True, message="XDM settings successfully saved.")
...
def status(request, plugin_id):
    ...
    proc = Popen([utils.xdm_control, "onestatus"], stdout=PIPE, stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.xdm_icon, 'rb') as f:
        icon = f.read()
