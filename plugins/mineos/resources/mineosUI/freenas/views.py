...
def stop(request, plugin_id):
    ...
    try:
        ...
        mineos = models.MineOS.objects.create(enable=False)
    ...
    cmd = "%s onestop" % utils.mineos_control
    pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE,
        shell=True, close_fds=True)
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('mineos_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'mineos', 'plugin_id': plugin_id },
    }
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.mineos_icon, 'rb') as f:
        icon = f.read()
