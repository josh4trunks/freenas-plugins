...
from gamezUI.freenas import forms, models, utils
...
def start(request, plugin_id):
    (gamez_key, gamez_secret) = utils.get_gamez_oauth_creds()

    url = utils.get_rpc_url(request)
    trans = OAuthTransport(url, key=gamez_key, secret=gamez_secret)
    ...
    try:
        gamez = models.Gamez.objects.order_by('-id')[0]
        gamez.enable = True
        gamez.save()
    except IndexError:
        gamez = models.Gamez.objects.create(enable=True)

    try:
        form = forms.GamezForm(gamez.__dict__, instance=gamez, jail_path=jail_path)
        form.is_valid()
        form.save()
    except ValueError:
        return HttpResponse(simplejson.dumps({
            'error': True,
            'message': ('Gamez data did not validate, configure '
                'it first.'),
            }), content_type='application/json')

    cmd = "%s onestart" % utils.gamez_control
...
def stop(request, plugin_id):
    ...
    try:
        ...
        gamez = models.Gamez.objects.create(enable=False)

    try:
        form = forms.GamezForm(gamez.__dict__,
            instance=gamez,
            jail_path=jail_path)
        form.is_valid()
        form.save()
    ...
    cmd = "%s onestop" % utils.gamez_control
...
def edit(request, plugin_id):
    ...
    """
    Get the Gamez object
    If it does not exist create a new entry
    """
    try:
        gamez = models.Gamez.objects.order_by('-id')[0]
    except IndexError:
        gamez = models.Gamez.objects.create()
    ...
    if request.method == "GET":
        form = forms.GamezForm(instance=gamez, jail_path=jail_path)
    ...
    form = forms.GamezForm(request.POST, instance=gamez, jail_path=jail_path)
    if form.is_valid():
        form.save()

        cmd = "%s restart" % utils.gamez_control
        pipe = Popen(cmd, stdin=PIPE, stdout=PIPE, stderr=PIPE, shell=True, close_fds=True)

        return JsonResponse(request, error=True, message="Gamez settings successfully saved.")
...
def treemenu(request, plugin_id):
    ...
    plugin = {
        ...
        'type': 'pluginsfcgi',
        'url': reverse('gamez_edit', kwargs={'plugin_id': plugin_id}),
        'kwargs': {'plugin_name': 'gamez', 'plugin_id': plugin_id },
    }
...
def status(request, plugin_id):
    ...
    proc = Popen(["/usr/local/etc/rc.d/gamez", "onestatus"],
        stdout=PIPE,
        stderr=PIPE)
...
def treemenu_icon(request, plugin_id):
    ...
    with open(utils.gamez_icon, 'rb') as f:
        icon = f.read()
